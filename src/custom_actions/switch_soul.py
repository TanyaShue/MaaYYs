...
class SwitchSoul(CustomAction):
    def run(self,
        ...
        for count in range(1, 21):
            time.sleep(0.5)
            if context.run_pipeline("点击分组", {"点击分组": {"timeout":500,"recognition": "OCR","action": "Click","expected": json_data["group_name"],"roi": [1085, 86, 162, 542]}}):
                break
            if count >= 5:
        ...
        for count in range(1, 10):
            ...
            detail = context.run_recognition("点击队伍",image, {"点击队伍": {"timeout":500,"recognition": "OCR","expected": json_data["team_name"],"roi": [567, 138, 288, 356]}})
            print(detail)
            if detail.box:
                roi=[detail.box.x-30,detail.box.y-30,500,80]
                context.run_pipeline("装备御魂", {"装备御魂": {"action": "Click","timeout":500,"recognition": "TemplateMatch","template": "装备御魂.png","roi": roi,"next": "点击确定"},"点击确定": {"action": "Click","timeout":100,"recognition": "OCR","expected": "确定","roi": [400,350,475,175]}})
                break
            if count >= 5:
