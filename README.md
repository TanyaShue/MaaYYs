...
> 本项目仅用于学习与交流，请自行承担使用风险
> 自动化行为可能受到游戏版本、模拟器环境、庭院皮肤、区服包名等因素影响
...
更详细的图文教程见 [doc/使用指南.md](doc/使用指南.md)；macOS PlayCover 的完整配置流程见 [doc/macOS PlayCover 使用手册.md](doc/macOS%20PlayCover%20使用手册.md)。

## 支持的资源

请在脚本右上角处自行修改对应区服：
<details>
<summary>当前已内置以下区服配置</summary>
...
任务能力以仓库中的 `tasks/*.json` 与 `resource_pack/*/pipeline` 为准

## 常见问题

<details>
<summary>脚本找不到模拟器怎么办？</summary>

先确认模拟器已经启动，并开启 ADB 调试。自动搜索失败时，可尝试打开任务管理器搜索"adb.exe"然后重启该进程
